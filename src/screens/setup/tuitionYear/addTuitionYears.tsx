...
const AddTuitionYears = ({handleClose, programItem, levelItem, tuitionFeeItem}: {handleClose: () => void, programItem: Program[], levelItem: Level[], tuitionFeeItem: Tuition[]}) => {
  const {notification} = App.useApp();
  const queryClient = useQueryClient();
  const addTuitionYearMutation = useMutation({ mutationFn: createUpdateTuitionYear});

 

  const handleAddTuitionYear = async  (values: FormikValues, resetForm: () => void) => {
  ...
  return (
    ...
    onSubmit={(values, {resetForm}) => handleAddTuitionYear(values, resetForm)}
...
export const EditTuitionYears = ({ item, programItem, levelItem, tuitionFeeItem, handleClose,}: {  item: TuitionYear; programItem: Program[]; levelItem: Level[], tuitionFeeItem: Tuition[]; handleClose: () => void;}) => {
  const queryClient = useQueryClient();   
  const { notification } = App.useApp();
  ...
  const editTuitionYearMutation = useMutation({
    ...
    const handleEditTuitionYear = async (values: FormikValues, resetForm: () => void) => {
      const payload: Partial<TuitionYear> = {
        id: item?.id,
        tuitionId: values?.tuition,
        levelId: values?.level,
  ...
  return (
    <Formik
      ...
      validationSchema={validationSchema}
      onSubmit={(values, {resetForm}) => handleEditTuitionYear(values, resetForm)}
      enableReinitialize
    >
      <Form className="fields">
