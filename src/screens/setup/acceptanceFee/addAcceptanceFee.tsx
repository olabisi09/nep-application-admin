...
import { getProgramTypes } from '../applicationFeeSetup/request';
...
const AddAcceptanceFee: FC<ComponentProps> = ({ record, handleClose }) => {
  ...
  const createUpdateApplicationFeeHandler = async (values: FormikValues, resetForm: () => void) => {
    const payload: Partial<ApplicationFee> & { acceptanceFee?: number } = {
      ...
      acceptanceFee: Number(values.acceptanceFee),
    };
  ...
  const validationSchema = Yup.object().shape({
    ...
    acceptanceFee: validator.amount,
  ...
  return (
    <Formik
      initialValues={{
        ...
        acceptanceFee: record?.acceptanceFee ?? 0,
        ...
        return (
          <Form>
            <section className="fields">
              ...
              <Input name="acceptanceFee" placeholder="0.00" label="Amount" />
...
export default AddAcceptanceFee;
