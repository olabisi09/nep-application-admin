...
const EditAdmissionRequirement = ({admissionRequirement, handleClose}:{ admissionRequirement: AdmissionRequirement; handleClose:()=> void}) => {
  ...
  const handleEditAdmissionRequirement = async (values: FormikValues, resetForm: () => void) => {
    const payload: Partial<AdmissionRequirement> = {
  ...
  >
    {({ setFieldValue }) => (
      <Form className="fields">
        ...
        <div className="btn-group">
          <Button  variant="text" text="Cancel" />
          <Button text="Update" isLoading={editAdmissionRequirementMutation.isPending} disabled={editAdmissionRequirementMutation?.isPending}/>
        </div>
      </Form>
    )}
  </Formik>
